...
arduinos = []
arduinos_usb_paths = [
    {"path": '/dev/cu.usbmodem11301', "inst": "bells"},
    {"path": '/dev/cu.usbmodem11101', "inst": "bells"},
    {"path": '/dev/cu.usbmodem11201', "inst": "bells"},
    {"path": '/dev/cu.usbmodem114301', "inst": "bells"},
    {"path": '/dev/cu.usbmodem114101', "inst": "ghungroo"},
    {"path": '/dev/cu.usbmodem114401', "inst": "wind"},
...
def analyze_trends():
    ...
    for key in trends:
        ...
        if len(trend) < 2:
            ...
            result.append(
                {"slope": slope, "intercept": intercept, "inst": key})
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        trend_index += 1
    return result
...
def actuate_motors(time):
    ...
    if not analysis:
        print("stop analysis from mototr actuation")
        return
    print(analysis)
    for value in analysis:
        ...
        inst = value["inst"]
        slope = value["slope"]
        this_arduinos = [item["arduino"]for item in arduinos if item["inst"] == inst]
